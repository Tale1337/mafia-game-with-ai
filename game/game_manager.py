...
class Game (BaseModel):
    ...
    def get_player_answer(self, player: Player) -> Message:
        if player.player_type == PlayerType.AI:
            system_prompt = PromptBuilder.get_system_prompt(player)
            user_prompt = PromptBuilder.get_user_prompt(self, player)
            content = get_ai_answer([
                {
                    "role": "system",
            ...
            content = input("Введите свой ответ: ")
